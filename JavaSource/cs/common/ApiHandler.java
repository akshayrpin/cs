...
public class ApiHandler {
	...
	public static String post(String url, String json) {
		return CsApi.post(url, json);
	}

	public static BrowserItemsVO getPanels(RequestVO vo) {
		Logger.highlight("#####################################################################################################1"+vo.getUrl());
		String json = post(vo);
	...
	public static TypeVO getType(RequestVO vo) {
		return CsApi.getType(vo);
	}

	public static TypeVO getType(String url) {
		return CsApi.getType(url);
	}
	...
	public static ObjGroupVO getGroup(RequestVO vo) {
		return CsApi.getGroup(vo);
	}
	
	public static ObjGroupVO[] getGroups(RequestVO vo) {
		return CsApi.getGroups(vo);
	}
	...
	public static SubObjVO[] getChoices(String choicetype, String entity, String type, int typeid) {
		return CsApi.getChoices(choicetype, entity, type, typeid);
	}
	...
	public static SubObjVO[] getLkupObj(String choicetype, String entity, String type, int selectedid) {
		return CsApi.getLkupObj(choicetype, entity, type, selectedid);
	}
	...
	public static SubObjVO[] getChoices(RequestVO vo) {
		return CsApi.getChoices(vo);
	}

	public static AvailabilityVO getAvailability(RequestVO vo) {
		return CsApi.getAvailability(vo);
	}

	public static InspectionStatisticsList getInspectionStatistics(RequestVO vo) {
		vo.setRequest("statistics");
		String json = post(vo);
		ObjectMapper mapper = new ObjectMapper();
		mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
		InspectionStatisticsList evo = new InspectionStatisticsList();
		try {
			evo = mapper.readValue(json, InspectionStatisticsList.class);
		}
		catch (Exception e) { Logger.error(e); }
		return evo;
	}
	...
	public static ResponseVO toResponseVO(String json) {
		return CsApi.toResponseVO(json);
	}
	...
	public static ResponseVO getResponseVO(RequestVO vo) {
	 ...
	 public static Token createToken(String entity, String username, String representative, String reppassword, String ip) {
		 Token r = new Token();
		 if (Operator.hasValue(username) && Operator.hasValue(representative) && Operator.hasValue(reppassword) && Operator.hasValue(ip)) {
			RequestToken rt = new RequestToken();
			rt.setUsername(username);
			rt.setIp(ip);
			rt.setRep(representative);
			rt.setReppassword(reppassword);
			StringBuilder sb = new StringBuilder();
			...
			sb.append("/users/createtoken");
			String url = sb.toString();
			sb = new StringBuilder();
			sb = null;
			String t = post(url, rt.toString());
			try {
				ObjectMapper mapper = new ObjectMapper();
				mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
				r = mapper.readValue(t, Token.class);
			}
			catch (Exception e) { e.printStackTrace(); Logger.error(e); }
		 }
		 return r;
	 ...
	 public static Token logout(String entity, String token, String ip) {
		 Token r = new Token();
		 if (Operator.hasValue(token) && Operator.hasValue(ip)) {
			RequestToken rt = new RequestToken();
			rt.setToken(token);
			rt.setIp(ip);
			StringBuilder sb = new StringBuilder();
	 ...
	 public static ResponseVO checkTabAccess(String entity, String token, String ip) {
		...
		if (Operator.hasValue(entity)) {
			...
			vo.setType(entity);
			vo.setGrouptype("ui");
			vo.setRequest("tabaccess");
			r = getResponseVO(vo);
		}
		return r;
	 }
	 
	 public static ResponseVO attach(RequestVO vo) {
			String json ="";
			String r = "";
		 	try {
		 		ObjectWriter ow = new ObjectMapper().writer().withDefaultPrettyPrinter();
				json = ow.writeValueAsString(vo);
				json = postAttachment(vo.getUrl(), r);
			}
		 	catch(Exception e) {
	 ...
	 public static String postAttachment(String url, String json) {
			Logger.info("POST URL", url);
			StringBuilder sb = new StringBuilder();
			try {
				HttpClient c = new DefaultHttpClient();
	 ...
	 public static String escapeSolr(String url){
		 if(Operator.hasValue(url)){
			//"+ - && || ! ( ) { } [ ] ^ " ~ * ? : \"
			 
			 /*url = Operator.replace(url, "{", "%7B");
			 url = Operator.replace(url, "}", "%7D");
			 url = Operator.replace(url, "\"\"", "%22");
			 
			 url = Operator.replace(url, " ", "\\%20\\");*/
			 //String s = " '\\', '+', '-', '&', '|', '!', '(', ')', '{', '}', '[', ']', '^', '~', '*', '?', ':', '\"\', ';', ' '";
		      /*  $replace = array('\\\\', '\\+', '\\-', '\\&', '\\|', '\\!', '\\(', '\\)', '\\{', '\\}', '\\[', '\\]', '\\^', '\\~', '\\*', '\\?', '\\:', '\\"', '\\;', '\\ ');
		        $string = str_replace($match, $replace, $string);
			 url = Operator.replace(url,"(","\(");*/
		 }
