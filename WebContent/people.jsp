...
String solrurl = CsConfig.getString("search.people");
...
String imgeditsrc = Config.rooturl() +""+ CsConfig.getImage("black", "edit");
...
<style>
	#resultsadd tr td { padding-bottom: 15px; padding-top: 15px; border-bottom: 1px solid #cccccc  }
</style>
...
//		$('#cssearch_results').hide();
	...
	function selectResult(pid) {
	 	var ht = $('#people_result_'+pid).html();
	 	var pname = $('#people_name_'+pid).html();
	 	var pusername = $('#people_username_'+pid).html();
	 	...
	 	selectPeople(pid, pname, pemail, pusername, pgroup, paddress, plic, ptype);
	}

	function selectPeople(pid, pname, pemail, pusername, pgroup, paddress, plic, ptype) {
	 	var pdisplay = pname + '<br/>';

	 	if (hasValue(pgroup)) {
	 		pdisplay +=  pgroup + '<br/>';
	 	}
	 	if (hasValue(pusername) && pemail.toLowerCase() != pusername.toLowerCase()) {
	 		pdisplay +=  pusername + '<br/>';
	 	}
	 	if (hasValue(pemail)) {
	 		pdisplay +=  pemail + '<br/>';
	 	}
	 	if (hasValue(paddress)) {
	 		pdisplay +=  paddress + '<br/>';
	 	}
	 	if (hasValue(ptype)) {
	 		pdisplay +=  ptype + '<br/>';
	 	}
		...
		var td = $('<td/>');
		td.addClass('people_result');
		td.html(pdisplay);

		var rmtd = $('<td/>');
		rmtd.addClass('people_result');
		rmtd.css({
			'width': '1%',
			'vertical-align': 'top',
			'cursor': 'pointer'
		...
		if (pemail && pemail != '') {
			var spanpri = $('<span/>');
			spanpri.attr('primary', 'false');
			spanpri.attr('prival', pid);
			spanpri.html('Not Primary');
			tdpri.append(spanpri);
			spanpri.click(function() {
		...
		var spanappl = $('<span/>');
		spanappl.attr('appl', 'false');
		spanappl.attr('applval', pid);
		spanappl.html('Copy as Applicant');
		tdappl.append(spanappl);
		spanappl.click(function() {
			setApplicant(pid);
		});
		...
		tr.append(tdappl);
		addChoice(pid);
	 	$('#people_choices').append(tr);
	 	$('#people_result_'+pid).hide();
	}
	...
	function containsAppl(pid) {
		var appl = $('#COPYAPPLICANT');
		var achoice = appl.val();
		if (achoice == pid) { return true; }
		var aarray = achoice.split('|');
		var idx = jQuery.inArray(pid, aarray);
		return idx > -1;
	}

	function removeApplicant(pid) {
		var newappl = $('span[applval='+pid+']');
		newappl.attr('appl','false');
		var appl = $('#COPYAPPLICANT');
		var cappl = appl.val();
		if (cappl == pid) { appl.val(''); }
		else {
			var achoice = '';
			var abool = false;
			var aarray = cappl.split('|');
	...
	function addApplicant(pid) {
		var newappl = $('span[applval='+pid+']');
		newappl.attr('appl','true');
		var appl = $('#COPYAPPLICANT');
		var cappl = appl.val();
		if (hasValue(cappl)) {
			cappl += '|';
	...
	function containsChoice(pid) {
		...
		var idx = jQuery.inArray(pid, parray);
		return idx > -1;
	}
	...
	function editpeople(pid) {
		$.fancybox({
			...
			transitionOut		: 'none',
			type				: 'iframe',
			href				: '<%= Config.fullcontexturl() %>/selectpeople.jsp?_ent=<%= entity %>&_type=<%= type %>&_typeid=<%= typeid %>&_id='+pid+'&_grptype=people',
			helpers: {
		    	overlay: {
		    		locked: false
		    	}
	...
	function displayresults(output){
		output = JSON.stringify(output);
 		...
 		$.each(resp['docs'], function(k,v) {
 			...
 			if(v.id!=''){
 					var hde = '';
 					if (containsChoice(v.id)) {
 						hde = ' style="display: none"';
 					}
					c+= '<tr'+hde+' id="people_result_'+v.id+'">';
 					c+= '<td width="1%" valign="top" nowrap>';
					c+= '<img src="'+imgsrc+'" style="cursor:pointer;" class="addpeople" id="'+v.id+'">';
					c+= '<br/>';
					c+= '<img src="'+imgeditsrc+'" style="cursor:pointer;" class="editpeople" id="'+v.id+'">';
 					c+= '</td>';
 					c+= '<td>';
					...
					var eml = v.email; if(eml==undefined){ eml = "";}
					var username = v.username; if(username==undefined){ username = "";}
					
					c += '<span class="people_result people_name" id="people_name_'+v.id+'">'+v.name+'</span>';
					if (hasValue(username) && eml.toLowerCase() != username.toLowerCase()) {
						c += '<span class="people_result people_username" id="people_username_'+v.id+'">'+username+'</span>';
					}
					if (hasValue(eml)) {
						c += '<span class="people_result people_email" id="people_email_'+v.id+'">'+eml+'</span>';
					}
		...
		<div id="csuimain">
			<div class="csuicontrol">
				<table cellpadding="0" cellspacing="0" border="0" width="100%">
					<tr>
						<td align="left">
							<table class="csui_tools" width="100%">
								<tr>
									<td>
									...
									<td align="right">
										<a target="lightbox-iframe" href="<%= Config.fullcontexturl() %>/selectpeople.jsp?_ent=<%= entity %>&_type=<%= type %>&_typeid=<%= typeid %>&_grptype=people"><img src="<%= ObjTables.WHITEADDIMGURL %>" height="25" width="25" border="0"/></a>&nbsp;&nbsp;
									</td>
								</tr>
							</table>
						</td>
					...
					<div id="csuimain">
						<div class="csuicontent">
							...
							<form id="peoplesearch">
								...
								<input type="hidden" id="qchange" name="qchange" value="">
										...
										<td class="csui">		
				 						...
				 						<td width="50%" valign="top">
											...
											<div id="div_choices" style="border-left: 1px solid #cccccc; padding-left: 20px">
												<form class="form" method="post" action="action.jsp" id="peoplesubmit" success="<%=req.summaryUrl()%>">
													<table cellpadding="5" cellspacing="0" border="0" width="90%">
														<tr>
															<td class="csui_defaulttitle">SELECTED</td>
														</tr>
													</table>
													...
													<input type="hidden" id="SET_PRIMARY_CONTACT" name="SET_PRIMARY_CONTACT" value=""/>
													<input type="hidden" id="COPYAPPLICANT" name="COPYAPPLICANT" value=""/>
													<input type="hidden" name="<%= RequestMapper.entity %>" value="<%= entity %>"/>
													<input type="hidden" name="<%= RequestMapper.entityid %>" value="<%= entityid %>"/>
													<input type="hidden" name="<%= RequestMapper.type %>" value="<%= type %>"/>
													<input type="hidden" name="<%= RequestMapper.typeid %>" value="<%= typeid %>"/>
													<input type="hidden" name="<%= RequestMapper.groupid %>" value=""/>
													<input type="hidden" name="<%= RequestMapper.group %>" value="people"/>
													<input type="hidden" name="<%= RequestMapper.grouptype %>" value="people"/>
												</form>
											</div>
