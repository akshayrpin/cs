...
//	nav.setRequest("details");
	...
	String usernamedesc = "Enter the username that the customer uses to access their online account. In most cases, the value entered here should be the same as the email address.";
	if (Operator.hasValue(dvo.getString("USERNAME"))) {
		editable = false;
		usernamedesc = "";
	}
	...
	<head>
		...
		<script>
			...
			$(document).ready(function() {
				...
				$('#confirmeditusername').click(function() {
					swal(
							{
								title: 'Edit Username?',
								text: 'WARNING: Editing the username of an account will affect all activities that this user is associated with!!!',
								type: "warning",
							...
							function(isConfirm) {
								if (isConfirm) {
				 					...
				 					$('#confirmeditusername').hide();
				 					var inp = $('input[name=USERNAME]');
				 					inp.attr('type','text');
				 					inp.addClass('csuiform');
								}
							}
						);
			...
			<div class="csuicontent">
				...
				<form id="editpeople" action="action.jsp" method="post">
						...
						<table class="csui">
							<tr>
							<% if (Operator.hasValue(dvo.getString("USERNAME")))  { %>
							<td class="csui_label" id="label_USERNAME" valign="top">USERNAME</td>
							<td class="csui vertical csui_field" id="field_USERNAME" valign="top">
								<input type="hidden" name="USERNAME" value="<%= dvo.getString("USERNAME") %>"/>
								<span id="usernamedisplay"><%= dvo.getString("USERNAME") %></span>
								<div id="confirmeditusername" style="float:right; cursor: pointer"><img src="/cs/images/icons/input/docedit.png"/></div>
							</td>
							<% } else { %>
						...
						<table class="csui">
							...
							<tr>
							<%= ObjTables.cells("COPYAPPLICANT", "COPY AS APPLICANT", "", "Boolean", "boolean", false, "csui", true) %>
