...
<%
	...
	tpvo.setType("project");
	tpvo.setGroup("parking");
	tpvo.setGrouptype("parking");
...
<head>
	...
	<script>
		$(document).ready(function() {
			...
			$('[name="LKUP_ACT_TYPE_ID"]').change(function() {
				var v = $(this).val();
				...
				$('#NO_OF_VEHICLES').change(function() {
					numVehicles();
// 					$('#vhhtml').empty();
...
// 					}
		...
		function numVehicles(num) {
			$('[rel=vehicle]').hide();
			
			try {
				var num = parseInt($('#NO_OF_VEHICLES').val());
				if($('#vovernight').val()==255){
					num = 1;
				}
				...
				for (i=0;i<num;i++) {
					$('#vh_'+i).show();
					if($('#vovernight').val()==255){
						$('#LICENSE_PLATE_'+i).attr("valrequired","true");
					}else {
						$('#LICENSE_PLATE_'+i).attr("valrequired","false");
					}
				}
				$('#<%=RequestMapper.subgroupsize%>').val(num);
			} catch(e) { }
		}
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<br/><br/>
				...
				<input type="hidden" name="customsize" id="customsize" value="0">
				<input type="hidden" name="<%= RequestMapper.subgroup %>" value="vehicle">	
				<input type="hidden" id="<%= RequestMapper.subgroupsize %>" name="<%= RequestMapper.subgroupsize %>" value="0">	
				
				<input type="hidden" name="vovernight" id="vovernight" value="0">	
				...
				<table class="csui" colnum="2" type="default">
					...
					<tr>
						...
						<td class="csui_label" colnum="2" alert="">NO OF VEHICLES</td>
						<td class="csui" colnum="2" type="String" itype="String" alert="" colspan="4">
						<select name="NO_OF_VEHICLES" id="NO_OF_VEHICLES"  itype="NO_OF_VEHICLES" valrequired="true"></select>
						</td>
					...
					<tr>
						...
						<td class="csui" colnum="2" type="DATE" itype="DATE" alert=""><input name="START_DATE" id="START_DATE" type="text" itype="date" value="<%=d.getString("YYYY/MM/DD") %>" valrequired="true" value="" maxchar="10000"></td>
						<td class="csui_label" colnum="2" alert="">EXP DATE</td>
						<td class="csui" colnum="2" type="DATE" itype="DATE" alert=""><input name="EXP_DATE" type="text" id="EXP_DATE"  itype="date" value="<%=d.getString("YYYY/MM/DD") %>" valrequired="true" value="" maxchar="10000"></td>
					</tr>
					<tr>
				...
				<table class="csui" type="horizontal" id="vhhtml">
						<tr>
							<td class="csui_header" width="1px">&nbsp;</td>
...
%>
						...
						<tr id="vh_<%=i%>" rel="vehicle" style="display: none">
							<td class="csui_header" width="1px"><%= i+1 %></td>
							<td class="csui"><input type="text" id="LICENSE_PLATE_<%=i%>" name="LICENSE_PLATE_<%=i%>" ></td>
							<td class="csui"><input type="text" id="REG_STATE_<%=i%>" name="REG_STATE_<%=i%>" maxlength="2"></td>
							<td class="csui"><input type="text" id="VEHICLE_MAKE_<%=i%>" name="VEHICLE_MAKE_<%=i%>"></td>
							<td class="csui"><input type="text" id="VEHICLE_MODEL_<%=i%>" name="VEHICLE_MODEL_<%=i%>"></td>
							<td class="csui"><input type="text" id="VEHICLE_YEAR_<%=i%>" name="VEHICLE_YEAR_<%=i%>" maxlength="4"></td>
							<td class="csui"><input type="text" id="VEHICLE_COLOR_<%=i%>" name="VEHICLE_COLOR_<%=i%>"></td>
						</tr>
<%
	}
%>
