...
<%@include file="search/gsuserfacet.jsp"%> 
...
<style>
	...
	$(document).ready(function() {
		...
		$('#peoplesubmit').submit(function(e) {
	    	...
	    	try { parent.addCollaborators(nchoice); } catch(e) { }
		...
		output = JSON.stringify(output);
 		...
 		$.each(resp['docs'], function(k,v) {
 			...
 			if(v.id!=''){
					...
					c += '<span class="people_result people_email" id="people_email_'+v.id+'">'+collemail+'</span>';
					c += '<span class="people_result people_phone" id="people_phone_'+v.id+'">'+ph+'</span>';
			 		c += '</div>';
 					c+= '</td>';
 					c+= '</tr>';
			}	
			...
			<div class="csuicontent">
				...
				<div>
					...
					<table cellpadding="0" cellspacing="0" border="0" align="center" width="100%">
						<tr>
							<td class="csui" valign="top">
								<table cellpadding="5" cellspacing="0" border="0" width="90%" id="cssearch_results">
									<tr>
										<td class="csui_defaulttitle">SEARCH RESULTS</td>
									</tr>
								</table>
	 						...
	 						<td width="50%" valign="top">
								...
								<div id="div_choices" style="border-left: 1px solid #cccccc; padding-left: 20px">
									<form id="peoplesubmit">
										<% if (collaborators.length > 0) { %>
										<table cellpadding="5" cellspacing="0" border="0" width="90%">
											<tr>
												<td class="csui_defaulttitle" colspan="2">CURRENTLY ASSOCIATED</td>
											</tr>
											<%
												for (int i=0; i<collaborators.length; i++) {
													SubObjVO coll = collaborators[i];
													String selected = "";
													if (coll.isSelected()) {
														selected = " checked";
													}
											%>
											<tr>
												<td width="1%" nowrap><input type="checkbox" name="CURRENT" value="<%= coll.getValue() %>"<%= selected %>></td>
												<td class="people_result"><%= coll.getText() %></td>
											</tr>
											<% } %>
										</table>
										<br/><br/>
										<% } %>
										<table cellpadding="5" cellspacing="0" border="0" width="90%" id="newselections" style="display: none">
											<tr>
												<td class="csui_defaulttitle">NEW SELECTIONS</td>
											</tr>
										</table>
										...
										<table cellpadding="5" cellspacing="0" border="0" width="100%">
											<tr>
												<td align="right"><input type="submit" name="action" value="insert"></td>
											</tr>
										</table>
										<input type="hidden" id="PEOPLE" name="PEOPLE" value=""/>
										<input type="hidden" name="<%= RequestMapper.id %>" value="<%= id %>"/>
										<input type="hidden" name="<%= RequestMapper.entity %>" value="<%= entity %>"/>
										<input type="hidden" name="<%= RequestMapper.entityid %>" value="<%= entityid %>"/>
										<input type="hidden" name="<%= RequestMapper.type %>" value="<%= type %>"/>
										<input type="hidden" name="<%= RequestMapper.typeid %>" value="<%= typeid %>"/>
