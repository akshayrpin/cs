...
<%
	...
	int id = map.getInt(RequestMapper.id, 0);
	...
	treq.setGrouptype(grouptype);
	treq.setRequest("team");
	SubObjVO[] team = ApiHandler.getChoices(treq);

	RequestVO ctreq = t.duplicate();
	ctreq.setGrouptype(grouptype);
	ctreq.setRequest("comboteam");
	ctreq.setId(Operator.toString(reviewrefid));
	SubObjVO[] comboteam = ApiHandler.getChoices(ctreq);

	String back = Config.fullcontexturl() + "/editreview.jsp?_id="+ id +"&_ent="+ entity +"&_type="+ type +"&_typeid="+ typeid +"&_grp="+ group +"&_grpid="+ groupid +"&_reviewid="+ reviewid +"&_revrefid="+ reviewrefid;
		...
		<div id="csuimain">
			...
			<div class="csuicontent">
				...
				<form class="form" ajax="no" id="editform" action="editreview.jsp" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="_act" value="saveteam">
