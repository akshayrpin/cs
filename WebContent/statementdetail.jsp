...
<%
	...
	nav.setAction("statementdetail");
	nav.setRequest("statementdetail");
	
	
	
	ResponseVO ru = new ResponseVO();
	ResponseVO r = ApiHandler.getResponse(nav);
	FeesGroupVO o = r.getType().getStatements()[0].getGroups()[0];
	
	if(map.equalsIgnoreCase("EXECUTE", "UPDATESDFEE")){
		HashMap<String, String> extras = new HashMap<String, String>();
		String input1 = map.getString("INPUT1",map.getString("INPUT1_H"));
		extras.put("INPUT1", input1);
		String input2 = map.getString("INPUT2",map.getString("INPUT2_H"));
		extras.put("INPUT2", input2);
		String input3 = map.getString("INPUT3",map.getString("INPUT3_H"));
		extras.put("INPUT3", input3);
		String input4 = map.getString("INPUT4",map.getString("INPUT4_H"));
		extras.put("INPUT4", input4);
		String input5 = map.getString("INPUT5",map.getString("INPUT5_H"));
		extras.put("INPUT5", input5);
		nav.setExtras(extras);
		nav.setRequest("updatestatementdetailfinance");
...
//			$('textarea').tinymce();
			...
			<%if(ru.getErrors().size()<=0 && map.equalsIgnoreCase("EXECUTE", "UPDATESDFEE")){%>
				parent.resetFees();
	...
	<div id="csuibody">
		<div id="csuimain">
			...
			<div class="csuicontent">
				<form id="csfeepick" action="statementdetail.jsp" method="post">
				...
				<input type="hidden" name="EXECUTE" value="UPDATESDFEE">
				<div class="csui_divider"></div>
							...
							for(int i=0;i<l;i++){ 
								...
								<tr>
									<td class="csui" colnum="2" type="String" itype="String" alert="">
										<%=name %>
										<input type="hidden" name="INPUT1_H" value="<%=fvo.getInput1()%>">
										<input type="hidden" name="INPUT2_H" value="<%=fvo.getInput2()%>">
										<input type="hidden" name="INPUT3_H" value="<%=fvo.getInput3()%>">	
										<input type="hidden" name="INPUT4_H" value="<%=fvo.getInput4()%>">	
										<input type="hidden" name="INPUT5_H" value="<%=fvo.getInput5()%>">	
									</td>
									
									<td class="csui" colnum="2" type="DATE" itype="DATE" alert="">
										<%if(fvo.getInputeditable1().equals("Y")){ %>
											<%=fvo.getInputtypelabel1() %>	<%=fvo.getInput1() %>
										</br>
										<%} %>
										<%if(fvo.getInputeditable2().equals("Y")){ %>
										<%=fvo.getInputtypelabel2() %>	<%=fvo.getInput2() %>
										</br>
										<%} %>
										<%if(fvo.getInputeditable3().equals("Y")){ %>
										<%=fvo.getInputtypelabel3() %>	<%=fvo.getInput3() %>
										</br>
										<%} %>
										<%if(fvo.getInputeditable4().equals("Y")){ %>
										<%=fvo.getInputtypelabel4() %>	<%=fvo.getInput4() %>
										</br>
										<%} %>
										<%if(fvo.getInputeditable5().equals("Y")){ %>
										<%=fvo.getInputtypelabel5() %>	<%=fvo.getInput5() %>
										</br>
										<%} %>
									</td>
									<td class="csui" colnum="2" type="DATE" itype="DATE" alert="">
										<%if(fvo.getInputeditable1().equals("Y")){ %>
											<%=fvo.getInputtypelabel1() %>	<input type="text" class="dec" name="INPUT1" value="<%=fvo.getInput1() %>" >
										</br>
										<%} %>
										<%if(fvo.getInputeditable2().equals("Y")){ %>
										<%=fvo.getInputtypelabel2() %>	<input type="text" class="dec"  name="INPUT2" value="<%=fvo.getInput2() %>" >
										</br>
										<%} %>
										<%if(fvo.getInputeditable3().equals("Y")){ %>
										<%=fvo.getInputtypelabel3() %>	<input type="text" class="dec"  name="INPUT3" value="<%=fvo.getInput3() %>" >
										</br>
										<%} %>
										<%if(fvo.getInputeditable4().equals("Y")){ %>
										<%=fvo.getInputtypelabel4() %>	<input type="text" class="dec"  name="INPUT4" value="<%=fvo.getInput4() %>" >
										</br>
										<%} %>
										<%if(fvo.getInputeditable5().equals("Y")){ %>
										<%=fvo.getInputtypelabel5() %>	<input type="text" class="dec"  name="INPUT5" value="<%=fvo.getInput5() %>" >
										</br>
										<%} %>
									</td>
								</tr>
					...
					<table class="csui">
					<tr>
						<td class="csui" ><h1><font color="red">NOTE *MODIFYING THE INPUT WILL RE-CREATE THE FEE AND TRANFER THE MONEY</font></h1></td>
					</tr>
				</table>
